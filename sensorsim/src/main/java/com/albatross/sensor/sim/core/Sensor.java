...
public abstract class Sensor extends AbstractMeasurer {
    ...
    public Quantity<?> generateNextQuantity() {
        ...
        if (this.distribution.getDistributionType()
                .equals(SimulationDataDistribution.DistributionCategoryType.GenericObjectDistribution)) {
            this.quantity = new Quantity(this.getType(), this.distribution.getNextObjectValue(), this.getSensorID());
            return this.quantity;
        }
        if (this.distribution.getDistributionType()
                .equals(SimulationDataDistribution.DistributionCategoryType.RecordedSequenceDistribution)) {
            this.quantity = new Quantity(this.getType(), this.distribution.getNextObjectValue(), this.getSensorID());
            return this.quantity;
        }
        return null;
